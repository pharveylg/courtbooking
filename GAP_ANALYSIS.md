...
| G-09 | Usage meters + events (§4 P2) | Already present: `platformTenants/{slug}/events` ledger + `usage/{period}` views | Billing meters unchanged (confirmed bookings + staff-reserve hrs); **B6 extends the ledger with participation events** — `queue_player_joined`, `queue_match_completed`, and `source`/`linkedOpenGameId`/`amountDue` on `booking_confirmed` + `playerEmail` on `open_play_join_pending` — so Open Play participation and queue walk-in volume are now measurable | CLOSED |
...
| G-18 | Platform overview / health (§5 Overview; §7) | None — no landing surface | Overview tab: tenants by state, live entitlements per product, MTD projected billing, open-invoice exposure, verified MTD, "needs attention" list **with one-click trial resolution (B3)**, **billing-status suggestions (B3)**, recent audit. **B4 adds a Diagnostics tab** (access explainer, mirror drift detection, per-tenant audit). **B6 adds participation analytics** (Open Play joins/participants/revenue, queue walk-ins and match player-slots) | PARTIAL (no background-job health — there are no jobs; no uptime surface) |
| G-19 | Platform Admin permission groups (`manage_organizations`, `manage_entitlements`, … §4 P1) | Single `superadmin` claim for everything | **CLOSED (Phase B5):** permission registry of 11 groups (roadmap §4 P1 + §12 billing families; `approve_platform_adjustment` and `view_platform_financial_reports` fold into `verify_platform_payment` / `view_platform_billing` until scale needs the split). Accounts carry a `platformPerms` array claim (`set-superadmin-claim.js --perms …`); `superadmin: true` remains the grandfathered owner shape. Enforced three places from one vocabulary: firestore.rules per collection, console tab visibility, and `requirePerm()` guards on every mutating service/handler — all covered by tests | CLOSED |
| G-20 | Support queue & diagnostics / "explain why" access (§4 P1) | Feature Ideas scratchpad doubles as support scoping; troubleshooting FAQ | **Diagnostics tab (B4):** per-tenant access explainer — lifecycle with provenance (who/when from audit), per-product entitlement → expected runtime state, mirror drift detection with one-click re-sync, recent tenant actions. Support *queue* remains the Feature Ideas scratchpad | PARTIAL (explainer shipped; queue still a scratchpad) |
...
| G-24 | Suspension blocks only the intended product scope (§6 cross-tenant tests) | Suspend = booking page only; Store/Tournament pages stay reachable when tenant suspended | **Decision implemented as a policy (B3):** `platformSettings/policy.suspendTenantAffectsAddons` — OFF (default) preserves the historical behavior; ON makes tenant suspension also gate Store & Tournaments via `deriveRuntimeExpectation()`. Applies to new changes immediately; retroactively via Diagnostics → Re-sync All. Covered by tests in both states | CLOSED (policy implemented, default = legacy) |
...
- **`tests/control-plane.test.js` — in-memory Firestore integration suite, 67/67 passed.** Covers: platform-state building with legacy migration fallbacks; entitlement grant → doc materialization + mirror + audit; lifecycle suspend/reactivate → runtime pause + `platformTenants`/`tenantDirectory` mirrors; suspend-preserves-Store behavior; starter-plan seeding; plan assignment → subscription + billing overwrite; draft invoice generation (frozen lines, overage math, add-on fees, zero-formula skip); issued → submitted → verified (never auto-verified); **B2:** historical-month generation buckets the right month's events (this test caught and fixed an offset bug) + month-offset math; **B1:** submission → queue → verify (attributed payment + reviewer stamp) and reject (reason stamped, visible to tenant); credit application; renderers; CSV export; **B3:** policy toggle gates/ungates add-ons on suspension through the shared derivation (both POLICY states tested), billing-status suggestion rules (overdue/grace/current, drafts excluded); **B4:** diagnostics renderer executes against drifted and clean mirrors; **B5:** owner vs scoped-staff claim evaluation (unknown perms filtered), scoped operators CAN run their family (entitlement grant) and ARE denied others (lifecycle, invoice verification); **B6:** participation metrics (joins, unique participants, revenue, queue walk-ins, match player-slots) computed from seeded events.
...
| B3 — Lifecycle policy engine | Suspension × add-ons **policy toggle** (`platformSettings/policy.suspendTenantAffectsAddons`, default = legacy behavior) applied through a single pure `deriveRuntimeExpectation()`; **one-click trial resolution** (activate/revoke) from the Overview attention list; **billing-status suggestions** engine (overdue invoice → overdue; open balance/negative prepaid → grace; all settled → current) with one-click apply | ✅ Shipped 11 Sep 2026 |
| B4 — Diagnostics workspace | Diagnostics tab: per-tenant access explainer (why online/paused/offline, lifecycle provenance from audit), entitlement → expected-runtime table, **mirror drift detection** with one-click re-sync (single tenant or all), recent tenant actions; `mirror-resync` audited | ✅ Shipped 11 Sep 2026 |
| B5 — Permission groups | 11-group **permission registry** (roadmap §4 P1 + §12); owner claim (`superadmin`) grandfathered, staff accounts carry a `platformPerms` array claim; enforced in rules per collection, console tab visibility, and `requirePerm()` service guards; `set-superadmin-claim.js --perms a,b,c` / `--list-perms`; entitlement ops no longer require organization-management rights (legacy status mirror made best-effort) | ✅ Shipped 11 Sep 2026 |
| B6 — Usage meter expansion | **Participation event capture** in the tenant app (additive only, no behavior change): queue joins + match completions as events, `booking_confirmed` now carries `source`/`linkedOpenGameId`/`amountDue`, `open_play_join_pending` carries `playerEmail`; reporting gains Open Play joins/participants/revenue, queue walk-in joins, and queue match player-slots (report card + CSV). Billing meters deliberately unchanged | ✅ Shipped 11 Sep 2026 |
