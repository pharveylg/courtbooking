...
| B5 — Permission groups | Split single `superadmin` claim into roadmap permission families (G-19) — when a second admin exists | Queued (blocked on real multi-admin need) |
| B6 — Usage meter expansion | Queue→booking conversion, Open Play participation (touches tenant app; event-schema change — deserves its own focused pass) | Queued |
